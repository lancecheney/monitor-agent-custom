...
pub async fn run(request: Request) -> Result<ResultMessage> {
    ...
    Ok(ResultMessage { request_id: request.request_id, province: request.province, rounds: request.rounds, items })
}
...
async fn trace(binary: &str, carrier: &'static str, label: &'static str, target: &str) -> Result<RouteItem> {
    let output = tokio::time::timeout(
        TRACE_TIMEOUT,
        Command::new(binary)
            .args(["-4", "-q", "3", "--parallel-requests", "1", "-m", "25", "-n", "--json", "--no-color", target])
            .stdin(Stdio::null()).output(),
    ).await.context("NextTrace timed out")??;
    ...
    for hop in &hops {
        for asn in hop_asns(hop) {
            if !known.contains(&asn) { known.push(asn); }
        }
    }
    let line = classify(carrier, &hops).unwrap_or_else(|| "未识别".into());
    Ok(RouteItem {
        carrier, carrier_label: label, target: target.into(), line,
        path: known.iter().map(|asn| format!("AS{asn}")).collect::<Vec<_>>().join(" → "),
        success: 1, rounds: 1, error: None,
    })
}

fn successful_hops(payload: &Value) -> Vec<Value> {
    payload.get("Hops").or_else(|| payload.get("hops")).and_then(Value::as_array).into_iter().flatten()
        .filter_map(|group| {
            let candidates: Vec<&Value> = group.as_array().map(|v| v.iter().collect()).unwrap_or_else(|| vec![group]);
            candidates.into_iter().find(|item| item.as_object().is_some() && item.get("Success").or_else(|| item.get("success")).and_then(Value::as_bool).unwrap_or(true)).cloned()
        }).collect()
}
...
fn hop_asns(hop: &Value) -> Vec<String> {
    ...
    while index + 2 < bytes.len() {
        if bytes[index].eq_ignore_ascii_case(&b'a') && bytes[index + 1].eq_ignore_ascii_case(&b's') {
            ...
            let mut end = start;
            while end < bytes.len() && bytes[end].is_ascii_digit() && end - start < 10 { end += 1; }
            if end - start >= 2 { out.push(text[start..end].to_owned()); }
            index = end;
        } else {
            index += 1;
        }
...
fn collect_numeric_key(value: &Value, key: &str, out: &mut Vec<String>) {
    match value {
        Value::Object(map) => for (name, value) in map {
            if name.eq_ignore_ascii_case(key) {
                let text = value.as_str().map(str::to_owned).unwrap_or_else(|| value.to_string());
                let digits: String = text.chars().filter(|c| c.is_ascii_digit()).collect();
                if !digits.is_empty() { out.push(digits); }
            }
            collect_numeric_key(value, key, out);
        },
        Value::Array(values) => for value in values { collect_numeric_key(value, key, out); },
        _ => {}
    }
}
...
fn inferred(ip: &str) -> Option<&'static str> {
    let entries = [
        ...
        ("10099", &["103.214.", "103.228.68.", "103.239.176.", "118.26.151.", "162.245.124.", "202.77.23.", "203.160.66.", "203.160.75."]),
        ("58807", &["223.120.", "223.119."]),
        ("9808", &["221.183.", "111.24.", "111.13."]),
    ];
    for (asn, prefixes) in entries { if prefixes.iter().any(|prefix| ip.starts_with(prefix)) { return Some(asn); } }
    let octets: Vec<_> = ip.split('.').collect();
    if octets.len() == 4 && octets[0] == "162" && octets[1] == "219" && matches!(octets[2].parse::<u8>(), Ok(32..=39 | 85)) { return Some("10099"); }
    None
}
...
fn records(hops: &[Value]) -> Vec<(Vec<String>, HashSet<String>)> {
    hops.iter().map(|hop| {
        ...
        for ip in &ips { if let Some(asn) = inferred(ip) { asns.insert(asn.into()); } }
        (ips, asns)
    }).collect()
}
...
fn classify(carrier: &str, hops: &[Value]) -> Option<String> {
    ...
    match carrier {
        "ct" => {
            let first = records.iter().position(|(ips, asns)| asns.contains("4809") || ips.iter().any(|ip| ip.starts_with("59.43.")))?;
            for (index, (ips, _)) in records.iter().enumerate().skip(first) {
                if ips.iter().any(|ip| ip.starts_with("59.43.245.")) && records.iter().skip(index + 1).any(|(later_ips, later_asns)| later_asns.contains("4134") || later_asns.contains("4847") || later_ips.iter().any(|ip| ip.starts_with(("202.97.", "202.96.", "219.141.", "219.142.", "106.37.")))) {
                    return Some("CN2GT".into());
                }
            }
            if records.iter().any(|(ips, asns)| asns.contains("23764") || ips.iter().any(|ip| ip.starts_with(("203.22.182.", "203.22.178.", "203.22.179.", "203.128.224.", "69.194.")))) { Some("CTGGIA".into()) } else { Some("CN2GIA".into()) }
        }
        "cu" => {
            let cug = records.iter().position(|(ips, asns)| asns.contains("10099") || ips.iter().any(|ip| inferred(ip) == Some("10099")));
            if let Some(index) = cug {
                let after = &records[index + 1..];
                if after.iter().any(|(_, a)| a.contains("9929")) { Some("CUG+9929".into()) }
                else if after.iter().any(|(_, a)| ["4837", "4808", "17816", "135061", "136958", "140979"].iter().any(|v| a.contains(*v))) { Some("CUG+4837".into()) }
                else { Some("CUG".into()) }
            } else if records.iter().any(|(_, a)| a.contains("9929")) { Some("9929".into()) }
            else if records.iter().any(|(_, a)| ["4837", "4808", "17816", "135061", "136958", "140979"].iter().any(|v| a.contains(*v))) { Some("4837".into()) }
            else { None }
        }
        "cm" => {
            let cmin2 = records.iter().any(|(_, a)| a.contains("58807"));
            let cmi = records.iter().any(|(_, a)| ["58453", "9808", "56040", "56041", "56042", "56044", "56045", "56046", "56047", "56048"].iter().any(|v| a.contains(*v)));
            match (cmin2, cmi) { (true, true) => Some("CMIN2+CMI".into()), (true, false) => Some("CMIN2".into()), (false, true) => Some("CMI".into()), _ => None }
        }
        _ => None,
    }
}
...
mod tests {
    ...
    fn hops(values: &[(&str, &str)]) -> Vec<Value> {
        values.iter().map(|(ip, asn)| serde_json::json!({"Address": ip, "Geo": {"asnumber": asn}, "Success": true})).collect()
    }
    ...
    fn classifies_three_carriers_from_ordered_paths() {
        assert_eq!(classify("ct", &hops(&[("59.43.1.1", "4809"), ("203.0.113.1", "4134")])), Some("CN2GIA".into()));
        assert_eq!(classify("cu", &hops(&[("103.214.1.1", "10099"), ("210.14.1.1", "9929")])), Some("CUG+9929".into()));
        assert_eq!(classify("cm", &hops(&[("223.120.1.1", "58807"), ("221.183.1.1", "9808")])), Some("CMIN2+CMI".into()));
    }
}
